(function (window) {
  ...
  var StateMachine = {
    ...
    JAVASCRIPT_STATE_MACHINE_PROPERTY_NAME: "Javascript State Machine",
    ...
    create: function(cfg, target) {
      ...
      var is_proto  = !!(target || cfg.target);	  
      ...
      for(var name in map) {
        if (map.hasOwnProperty(name))
          fsm[name] = StateMachine.buildEvent(name, map[name], is_proto ? undefined : fsm); // if prototype, don't pass as context since this will cause all state machines to share the context
      }
    ...
    buildEvent: function(name, map, context) {
      return function() {
        var fsm   = isStateMachine(this) ? this : context;
		if (fsm === undefined) throw ": Event called with invalid `this` - try wrapping call in function: setTimeout(function () { sm.event(); }, 1000);";
        ...
        if (false === StateMachine.beforeEvent(fsm, name, from, to, args))
        ...
        var leave = StateMachine.leaveState(fsm, name, from, to, args);
        if (false === leave) {
          fsm.transition = null;
          return StateMachine.CANCELLED;
        }
          ...
          if (fsm.transition)
            fsm.transition(); // in case user manually called transition() but forgot to return ASYNC
  ...
  var isStateMachine = function (obj) {
    return obj && obj[StateMachine.JAVASCRIPT_STATE_MACHINE_PROPERTY_NAME] === StateMachine.VERSION;
  };

  var setIsStateMachine = function (obj) {
    obj[StateMachine.JAVASCRIPT_STATE_MACHINE_PROPERTY_NAME] = StateMachine.VERSION;
  };
